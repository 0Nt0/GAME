...
func task_wander_around(task):
	if !CanSeePlayer.SeePlayer():
		if wonder.get_time()==0:
			...
			if vectorWond.y>0 :
				if vectorWond.x>0:
					animation.play("Slime_Right")
				elif vectorWond.x<0:
					animation.play("Slime_Left")
			elif vectorWond.y<0:
				if vectorWond.x>0:
					animation.play("Slime_Right")
				elif vectorWond.x<0:
				...
				animation.play("Slime_Up")
			elif vectorWond.y==0 && vectorWond.x<0:
				animation.play("Slime_Down")
			wonder.start_time();
			task.succeed()
	else:
		task.failed();	
...
func _on_AnimationPlayer_animation_finished(anim_name):
	match anim_name:
		"Slime_die":
				queue_free()
		"Slime_Hit":
			$AnimationPlayer.play("Slime_Down")
